...
class PandoraClient {
 public:
  PandoraClient(ros::NodeHandle node, ros::NodeHandle nh) {
    ...
    std::string lidarTopic = "/pandora/sensor/pandora/hesai40/PointCloud2";
    std::string cameraTopics[5];
    cameraTopics[0] = std::string("/pandora/sensor/pandora/camera/front_color");
    cameraTopics[1] = std::string("/pandora/sensor/pandora/camera/front_gray");
    cameraTopics[2] = std::string("/pandora/sensor/pandora/camera/right_gray");
    cameraTopics[3] = std::string("/pandora/sensor/pandora/camera/back_gray");
    cameraTopics[4] = std::string("/pandora/sensor/pandora/camera/left_gray");
    bool enableCamera = true;
    int timezone = 8;
    std::string frameId = std::string("hesai40");

    // parse nodehandle param
    bool ret = parseParameter(nh, &ip, &lidarRecvPort,
    ...
    psdk = new apollo::drivers::hesai::Pandora(ip, lidarRecvPort, gpsRecvPort,
            boost::bind(&PandoraClient::lidarCallback, this, _1, _2),
            NULL, startAngle * 100, pandoraCameraPort,
            boost::bind(&PandoraClient::cameraCallback,
                        this, _1, _2, _3, _4),
 ...
 private:
  ...
  apollo::drivers::hesai::Pandora *psdk;
};
